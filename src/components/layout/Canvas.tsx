    ...
    try {
      import('../../lib/canvas/nodeFactory').then(({ createLofiLayoutNode }) => {
        try {
          ...
          console.error('Error creating lo-fi layout node:', innerError);
        }
      }).catch(importError => {
        console.error('Error importing nodeFactory:', importError);
      });
    } catch (error) {
      console.error('Error in handleAddLofiLayoutNode:', error);
    }
  }, []);
